...
Try {
    ...
    $Registry = Get-ItemProperty -Path $Path -Name $AttrName3 -ErrorAction Silentlycontinue | Select-Object -ExpandProperty $AttrName
    If ($Registry -eq $AttrValue){
        Write-Output "Compliant"
        Exit 0
    } Else {
    ...
    $Registry = Get-ItemProperty -Path $Path1 -Name $AttrName4 -ErrorAction Silentlycontinue
    If ($Registry -eq $AttrValue){
        Write-Output "Compliant"
        Exit 0
    } Else {
