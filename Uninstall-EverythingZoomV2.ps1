...
#Close Zoom
#Get-Process Zoom | Foreach-Object { $_.CloseMainWindow() | Out-Null } | stop-process –force -erroraction SilentlyContinue
Stop-Process -ProcessName Zoom -Force -ErrorAction SilentlyContinue
...
function Uninstall-ZoomLocalMachine() {
    ...
    $registryPath = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{C6095BA2-96D7-478C-922F-01849BD21AFD}"
    if (Test-Path -Path $registryPath) {
        $installedZoomApps = Get-ChildItem -Path $registryPath -Recurse | Get-ItemProperty | Where-Object {$_.Publisher -like "Zoom*" } | Select-Object Displayname
